...
class conresnet(nn.Module):
    ...
    def forward(self, x_list):
        ...
        ## decoder
        res_x4_d = F.interpolate(x_res_d, size=(int(self.shape[0] / 4), int(self.shape[1] / 4), int(self.shape[2] / 4)),
                                 mode='trilinear', align_corners=True)  # [2,1,32,32,32]
        res_x4_h = F.interpolate(x_res_h, size=(int(self.shape[0] / 4), int(self.shape[1] / 4), int(self.shape[2] / 4)),
                                 mode='trilinear', align_corners=True)  # [2,1,32,32,32]
        res_x4_w = F.interpolate(x_res_w, size=(int(self.shape[0] / 4), int(self.shape[1] / 4), int(self.shape[2] / 4)),
                                 mode='trilinear', align_corners=True)  # [2,1,32,32,32]
        seg_x4 = F.interpolate(x, size=(int(self.shape[0] / 4), int(self.shape[1] / 4), int(self.shape[2] / 4)),
                               mode='trilinear', align_corners=True)  # [2,128,32,32,32]
        ...
        res_x2_d = F.interpolate(res_x4_d,
                                 size=(int(self.shape[0] / 2), int(self.shape[1] / 2), int(self.shape[2] / 2)),
                                 mode='trilinear', align_corners=True)  # [2,64,64,64,64]
        res_x2_h = F.interpolate(res_x4_h,
                                 size=(int(self.shape[0] / 2), int(self.shape[1] / 2), int(self.shape[2] / 2)),
                                 mode='trilinear', align_corners=True)  # [2,64,64,64,64]
        res_x2_w = F.interpolate(res_x4_w,
                                 size=(int(self.shape[0] / 2), int(self.shape[1] / 2), int(self.shape[2] / 2)),
                                 mode='trilinear', align_corners=True)  # [2,64,64,64,64]
        seg_x2 = F.interpolate(seg_x4, size=(int(self.shape[0] / 2), int(self.shape[1] / 2), int(self.shape[2] / 2)),
                               mode='trilinear', align_corners=True)  # [2,64,64,64,64]

        seg_x2 = seg_x2 + skip2  # [2,64,64,64,64]
        seg_x2, res_x2_d, res_x2_h, res_x2_w = self.seg_x2([seg_x2, res_x2_d, res_x2_h, res_x2_w])  # [2,32,64,64,64]

        res_x1_d = F.interpolate(res_x2_d,
                                 size=(int(self.shape[0] / 1), int(self.shape[1] / 1), int(self.shape[2] / 1)),
                                 mode='trilinear', align_corners=True)  # [2,32,128,128,128]
        res_x1_h = F.interpolate(res_x2_h,
                                 size=(int(self.shape[0] / 1), int(self.shape[1] / 1), int(self.shape[2] / 1)),
                                 mode='trilinear', align_corners=True)  # [2,32,128,128,128]
        res_x1_w = F.interpolate(res_x2_w,
                                 size=(int(self.shape[0] / 1), int(self.shape[1] / 1), int(self.shape[2] / 1)),
                                 mode='trilinear', align_corners=True)  # [2,32,128,128,128]
        seg_x1 = F.interpolate(seg_x2, size=(int(self.shape[0] / 1), int(self.shape[1] / 1), int(self.shape[2] / 1)),
                               mode='trilinear', align_corners=True)  # [2,32,128,128,128]

        seg_x1 = seg_x1 + skip1  # [2,32,128,128,128]
